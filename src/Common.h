...
TextureData loadImg(const char* const imgFilePath)
{
	...
	// Force stb_image to output 4 channels (RGBA)
	stbi_uc* data = stbi_load(imgFilePath, &texture.width, &texture.height,
		&originalComponents, 4);

	if (data == nullptr)
	{
		std::cerr << "Failed to load image: " << imgFilePath << "\n";
		texture.data = nullptr;
		return texture;
	}

	std::cout << "Load Image (" << texture.width << ", "
		<< texture.height << ", original=" << originalComponents
		<< ", output=4)" << std::endl;

	size_t dataSize = texture.width * texture.height * 4; // RGBA
	...
	for (int y = 0; y < texture.height / 2; ++y)
	{
		...
		{
			size_t top = (y * texture.width + x) * 4;
			size_t bottom = ((texture.height - 1 - y) * texture.width + x) * 4;

			std::swap(texture.data[top + 0], texture.data[bottom + 0]); // R
			std::swap(texture.data[top + 1], texture.data[bottom + 1]); // G
			std::swap(texture.data[top + 2], texture.data[bottom + 2]); // B
			std::swap(texture.data[top + 3], texture.data[bottom + 3]); // A
		}
	}
